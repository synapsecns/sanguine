...
// Parser parses events from the bonding manager contract.
...
func (p parserImpl) ParseSnapshotAccepted(log ethTypes.Log) (_ types.Snapshot, domain uint32, ok bool) {
	bondingManagerSnapshot, err := p.filterer.ParseSnapshotAccepted(log)
	if err != nil {
		return nil, 0, false
	}

	snapshot, err := types.DecodeSnapshot(bondingManagerSnapshot.SnapPayload)
	if err != nil {
		return nil, 0, false
	}

	return snapshot, bondingManagerSnapshot.Domain, true
}
