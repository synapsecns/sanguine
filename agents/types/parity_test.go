...
func TestEncodeChainGasParity(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	...
	//etherPrice := gofakeit.Uint16()
	//markup := gofakeit.Uint16()
	//gasDomain := gofakeit.Uint32()
	gasPrice := uint16(0)
	dataPrice := uint16(0)
	execBuffer := uint16(0)
	amortAttCost := uint16(0)
	etherPrice := uint16(0)
	markup := uint16(0)
	gasDomain := uint32(0)

	gasData := types.NewGasData(gasPrice, dataPrice, execBuffer, amortAttCost, etherPrice, markup)
	encodedGasData, err := types.EncodeGasData(gasData)
	Nil(t, err)

	encodedGasDataBigInt := new(big.Int).SetBytes(encodedGasData)

	goChainGas, err := types.EncodeChainGas(types.NewChainGas(gasData, gasDomain))
	Nil(t, err)

	goChainGasBigInt := new(big.Int).SetBytes(goChainGas)

	encodedContractGasData, err := gasDataContract.EncodeChainGas(&bind.CallOpts{Context: ctx}, encodedGasDataBigInt, gasDomain)
	Nil(t, err)

	Equal(t, encodedContractGasData.Bytes(), goChainGasBigInt.Bytes())

	goSnapGasHash := crypto.Keccak256Hash(goChainGas)
	goSnapHashBytes := goSnapGasHash.Bytes()

	var goSnapHashBytesB32 [32]byte
	copy(goSnapHashBytesB32[:], goSnapHashBytes[:])

	contractSnapGasHash, err := gasDataContract.SnapGasHash(&bind.CallOpts{Context: ctx}, []*big.Int{encodedGasDataBigInt})
	Nil(t, err)

	Equal(t, goSnapHashBytesB32, contractSnapGasHash)
}
