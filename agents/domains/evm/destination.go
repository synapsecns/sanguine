...
import (
	"context"
	"fmt"
	evmClient2 "github.com/synapsecns/sanguine/ethergo/client"
	"math/big"
	"time"
...
//
//nolint:staticcheck
func NewDestinationContract(ctx context.Context, client evmClient2.EVMChainID, destinationAddress common.Address) (domains.DestinationContract, error) {
...
func (a destinationContract) MessageStatus(ctx context.Context, message types.Message) (bool, error) {
	messageLeaf, err := message.ToLeaf()
	if err != nil {
		return false, fmt.Errorf("could not get message leaf: %w", err)
	}
