...
// DelinquentMessage is the format of a message from the DB that was not received.
type DelinquentMessage interface {
	// Origin is the origin chainID of the message.
	Origin() uint32
	// Sender is the sender of the message.
	Sender() common.Hash
	// Nonce is the nonce of the message.
	Nonce() uint32
	// Destination is the destination chainID of the message.
	Destination() uint32
	// Recipient is the recipient of the message.
	Recipient() common.Hash
}

// delinquentMessage is the implementation of a delinquent message.
type delinquentMessage struct {
	origin      uint32
	sender      common.Hash
	nonce       uint32
	destination uint32
...
// NewDelinquentMessage creates a new delinquent message.
func NewDelinquentMessage(origin uint32, sender common.Hash, nonce uint32, destination uint32, recipient common.Hash) DelinquentMessage {
	return delinquentMessage{
		origin:      origin,
		sender:      sender,
		nonce:       nonce,
...
func (d delinquentMessage) Origin() uint32 {
	return d.origin
}

func (d delinquentMessage) Sender() common.Hash {
	return d.sender
}

func (d delinquentMessage) Nonce() uint32 {
	return d.nonce
}

func (d delinquentMessage) Destination() uint32 {
	return d.destination
}

func (d delinquentMessage) Recipient() common.Hash {
	return d.recipient
}
