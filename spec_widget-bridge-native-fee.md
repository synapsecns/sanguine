...
Show a non-zero bridge quote `nativeFee` in the widget so users can see the additional origin-chain native asset cost before submitting, without changing the current bridge form flow or CTA behavior.
...
4. The `Bridge fee` row value must use the origin chain native currency symbol from widget chain metadata and display as `<amount> <symbol>`.
5. The `Bridge fee` row must be hidden when:
   - there is no valid quote,
   - the quote is loading,
   - `nativeFee` is absent, invalid, or equal to zero.
6. The `Bridge fee` row must be positioned with the other quote details in the expanded receipt and should appear after `Send` and before `Receive`.
7. The displayed `Bridge fee` amount must not collapse a non-zero fee into a misleading `0 <symbol>` or `0.0000 <symbol>` value.
8. Existing bridge execution, approval flow, and button enabled/disabled behavior must remain unchanged.
...
2. Implement receipt-side formatting for native fee display using origin chain native symbol.
3. Add the conditional `Bridge fee` row to the expanded receipt between `Send` and `Receive`.
4. Verify that zero-fee routes do not show the row and non-zero-fee routes do.
5. Run widget package lint/build validation and perform manual UI verification with at least one zero-fee and one non-zero-fee route.
...
- The `Send` row remains the input token amount only.
