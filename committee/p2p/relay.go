// TOODO: just messing around w/ hole punching. This/any dependencies on charon won't ship
...
func NewRelays(ctx context.Context, relayAddrs []string, lockHashHex string,
) ([]*p2p.MutablePeer, error) {
	var resp []*p2p.MutablePeer
	for _, relayAddr := range relayAddrs {
		...
		resp = append(resp, p2p.NewMutablePeer(p2p.NewRelayPeer(*info)))
	}
...
func resolveRelay(ctx context.Context, rawURL, lockHashHex string, callback func(p2p.Peer)) {
	var (
		prevAddrs      string
...
func multiAddrFromENRStr(enrStr string) (ma.Multiaddr, error) {
	...
	id, err := p2p.PeerIDFromKey(r.PubKey)
	if err != nil {
