...
WORKDIR /app/committee

RUN --mount=type=cache,target=/root/go/pkg/mod GOPROXY=https://proxy.golang.org go mod download -x
RUN --mount=type=cache,target=/root/go/pkg/mod  --mount=type=cache,target=/root/.cache/go-build CC=gcc CXX=g++ go build -tags=netgo,osusergo -ldflags="-s -w -extldflags '-static'" -o /app/bin/committee  main.go

FROM ubuntu:20.04

RUN apt update && apt install -y bash sqlite3 ca-certificates htop
RUN update-ca-certificates
COPY --from=builder /app/bin/committee /usr/local/bin
