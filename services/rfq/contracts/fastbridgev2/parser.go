...
type parserImpl struct {
	filterer *FastBridgeV2Filterer
}

// NewParser creates a new parser for the fastbridge contract.
func NewParser(fastBridgeAddress common.Address) (Parser, error) {
	parser, err := NewFastBridgeV2Filterer(fastBridgeAddress, nil)
	if err != nil {
		return nil, fmt.Errorf("could not create %T: %w", FastBridgeV2Filterer{}, err)
	}
