...
func (i *IntegrationSuite) TestRateLimit() {
	go func() {
		_ = i.relayer.Start(i.GetTestContext())
	}()
	go func() {
	...
	// we will have one large bridge over $10k usd,
	// we simply need to check when the rfq transaction will be included
	// we will see when the BridgeRequested event will be emitted, or simply wait for the receipt
	// Then, we will check successive block numbers to see when rfq gets filled
	// we should probably set the block wait to be larger so we can easily determine if the rate limi
	// works correcltly. anyways
	// then we will try and do the sliding window method where we check if $10k cumulative over the
	// block window will be rate limited after the most recent block. this is the freaky one
	// and tbh probably doesn't work.

	// Send ETH to the relayer on destination
	const initialBalance = 1000 //  lot of eth
	i.destBackend.FundAccount(i.GetTestContext(), i.relayerWallet.Address(), *big.NewInt(initialBalance))

	// let's give the user some money as well
	const userWantAmount = 100
	i.originBackend.FundAccount(i.GetTestContext(), i.userWallet.Address(), *big.NewInt(userWantAmount))
	...
	receipt, err := i.originBackend.TransactionReceipt(i.GetTestContext(), tx.Hash())
	i.NoError(err)
	i.Greater(receipt.BlockNumber, nil)
	// we have the receipt block number, now we need to see when the relayer picks it up
}
