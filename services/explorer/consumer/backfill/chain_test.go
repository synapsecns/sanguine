...
func (b *BackfillSuite) TestBackfill() {
	...
	lastBlockStored, err := b.db.GetUint64(b.GetTestContext(), fmt.Sprintf("SELECT ifNull(%s, 0) FROM last_blocks WHERE %s = %d", sql.BlockNumberFieldName, sql.ChainIDFieldName, uint32(testChainID.Uint64())))
	Nil(b.T(), err)
