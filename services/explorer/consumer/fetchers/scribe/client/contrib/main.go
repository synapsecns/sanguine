...
func main() {
	...
	err = os.Chdir(filepath.Join(root.Path, "services/explorer/consumer/fetchers/scribe/client/"))
	if err != nil {
		log.Fatalf("Error: %s", err.Error())
	}

	cfg, err := config.LoadConfig(filepath.Join(root.Path, "services/explorer/consumer/fetchers/scribe/client/gqlgen.yaml"))
	if err != nil {
		fmt.Fprintln(os.Stderr, "failed to load config", err.Error())
		os.Exit(2)
	}
	gqlgencConfig, err := clientConfig.LoadConfig(filepath.Join(root.Path, "services/explorer/consumer/fetchers/scribe/client/.gqlgenc.yaml"))
	if err != nil {
