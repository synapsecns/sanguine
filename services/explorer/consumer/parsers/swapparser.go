...
import (
	...
	swapFetcher "github.com/synapsecns/sanguine/services/explorer/consumer/fetchers/swap"
...
type SwapParser struct {
	...
	swapService swapFetcher.ISwapFetcher
...
func NewSwapParser(consumerDB db.ConsumerDB, swapAddress common.Address, metaSwap bool, consumerFetcher scribe.IScribeFetcher, swapService swapFetcher.ISwapFetcher, tokenDataService token.ITokenFetcher, tokenPriceService price.IPriceFetcher) (*SwapParser, error) {
	...
	if !core.IsTest() {
		for i := range totalTokenIndexRange {
			...
			g.Go(func() error {
				...
				tokenData, err = p.tokenDataService.GetPoolTokenData(groupCtx, tokenIndex, p.swapService)
				if err != nil {
					logger.Errorf("could not get token data: %v", err)
					return fmt.Errorf("could not get swap token data: %w", err)
				}
...
// GetCorrectSwapFee returns the correct swap fee for the given swap contract.
