...
func NewCCTPParser(consumerDB db.ConsumerDB, cctpAddress common.Address, consumerFetcher scribe.IScribeFetcher, backend bind.ContractBackend, tokenDataService token.ITokenFetcher, tokenPriceService price.IPriceFetcher, fromAPI bool) (*CCTPParser, error) {
...
func (c *CCTPParser) MatureLogs(ctx context.Context, cctpEvent *model.CCTPEvent, iFace cctpTypes.EventLog, chainID uint32) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("could not get swap token data: %w", err)
	}
	...
	err = retry.WithBackoff(context.Background(), func(ctx context.Context) error {
		return c.consumerDB.StoreEvent(ctx, &bridgeEvent)
	})
	if err != nil {
