...
func (p *ParserSuite) SetupTest() {
	...
	bcf, err := fetcher.NewBridgeConfigFetcher(p.bridgeConfigContract.Address(), p.bridgeConfigContract)
	Nil(p.T(), err)
	...
	p.consumerFetcher = fetcher.NewFetcher(p.gqlClient, p.metrics)
