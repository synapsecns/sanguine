...
// originToDestCol + generateDeDepQuery() + originToDestJoins.
...
func generateAllBridgeEventsQueryFromOrigin(chainID *int, address, tokenAddress, txHash *string, page int, in bool) string {
	...
	finalQuery := fmt.Sprintf("SELECT %s FROM (%s) %s", originToDestCol, generateDeDepQuery(compositeFilters, &pageValue, &pageOffset), originToDestJoins)
	return finalQuery
}
...
func generateAllBridgeEventsQueryFromDestination(chainID *int, address, tokenAddress, kappa, txHash *string, page int, in bool) string {
	...
	pageValue := page
	pageOffset := (page - 1) * sql.PageSize
	finalQuery := fmt.Sprintf("SELECT %s FROM (%s) %s", destToOriginCol, generateDeDepQuery(compositeFilters, &pageValue, &pageOffset), destToOriginJoins)
	return finalQuery
}
...
// nolint:gocognit,cyclop
func (r *queryResolver) GetBridgeTxsFromOrigin(ctx context.Context, chainID *int, address *string, txHash *string, page int, tokenAddress *string, pending bool) ([]*model.BridgeTransaction, error) {
	var err error
	var results []*model.BridgeTransaction
