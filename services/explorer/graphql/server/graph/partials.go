...
WITH baseQuery AS (SELECT *
     ...
     swapDeDup AS (SELECT if(amount_id >= 0 and tokens_sold = 0, toUInt256(amount_id), sold_id)     AS sold_id,
                   ...
                   FROM (
                            ...
                            FROM swap_events
                            LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)), map(
        1, 'Ethereum',
        10, 'Optimism',
        25, 'Cronos',
        56, 'BSC',
        ...
        1313161554, 'Aurora',
        1666600000, 'Harmony',
        7700, 'Canto') AS chainIDs


SELECT toString(date) AS date, mapUpdate(results, map('total', arraySum(mapValues(results)))) as date_results
FROM (
         SELECT date, maxMap(map(chain_id, total)) AS results
         FROM (SELECT b.date AS date, chainIDs[b.pre_fchain_id] as chain_id, (b.usdTotal + s.usdTotal) as total
               FROM (
                        ...
                        FULL OUTER JOIN (
                   SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                          chain_id,
                          sumKahan(multiIf(event_type = 0, amount_usd[sold_id], event_type = 1,
                                           arraySum(mapValues(amount_usd)), event_type = 9,
                                           arraySum(mapValues(amount_usd)), event_type = 10, amount_usd[sold_id], 0)
                              )                                        as usdTotal
                   FROM (SELECT *
                         FROM swap_events
                         LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                   group by date, chain_id
                   ) s ON b.date = s.date AND b.pre_fchain_id = s.chain_id
                  )
         group by date)
...
SELECT date, mapUpdate(results, map('total', arraySum(mapValues(results))))  as date_results
FROM (
         SELECT date, maxMap(map(chain_id, total)) AS results
         FROM (SELECT b.date AS date, chainIDs[b.chain_id] as chain_id, (b.usdTotal + s.usdTotal + m.usdTotal) as total
               FROM (
                        SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                               chain_id,
                              sumKahan(fee_usd)                        as usdTotal
                        FROM (SELECT *
                              FROM explorer_prod.bridge_events
                              LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                        GROUP BY date, chain_id) b

                        FULL OUTER JOIN (
                   SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                          chain_id,
                          sumKahan(arraySum(mapValues(admin_fee_usd)))                      AS usdTotal
                   FROM (SELECT *
                         FROM explorer_prod.swap_events
                         LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                   group by date, chain_id
                   ) s ON b.date = s.date AND b.chain_id = s.chain_id
                   ...
                   SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                          ...
                          sumKahan(fee_usd)                 AS usdTotal
                   FROM (SELECT *
                         FROM explorer_prod.message_bus_events
                         LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                   group by date, chain_id
                   ) m ON b.date = m.date AND b.chain_id = m.chain_id
                  )
         group by date)
group by date, results
order by date desc
...
FROM (
         ...
         FROM (SELECT b.date AS date, chainIDs[b.chain_id] as chain_id, (b.usdTotal + s.usdTotal + m.usdTotal) as total
               FROM (
                        SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                               chain_id,
                               uniq(chain_id, sender)                      as usdTotal
                        FROM (SELECT *
                              FROM explorer_prod.bridge_events
                              LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                        GROUP BY date, chain_id) b
...
FROM (
         ...
         FROM (SELECT b.date AS date, chainIDs[b.chain_id] as chain_id, (b.usdTotal + s.usdTotal + m.usdTotal) as total
               FROM (
                        SELECT toDate(FROM_UNIXTIME(timestamp, '%Y/%m/%d')) as date,
                               chain_id,
                               uniq(chain_id, tx_hash)                      as usdTotal
                        FROM (SELECT *
                              FROM explorer_prod.bridge_events
                              LIMIT 1 BY chain_id, contract_address, event_type, block_number, event_index, tx_hash)
                        GROUP BY date, chain_id) b
