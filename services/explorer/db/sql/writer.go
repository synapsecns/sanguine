...
func (s *Store) StoreLastBlock(ctx context.Context, chainID uint32, blockNumber uint64, contractAddress string) error {
	...
	if blockNumber > entry.BlockNumber {
		...
		dbTx = s.db.WithContext(ctx).Exec("set mutations_sync = 2")
		if dbTx.Error != nil {
			return fmt.Errorf("could not set mutation sync for block %d on chain %d: %w", blockNumber, chainID, dbTx.Error)
		}
		...
		dbTx = s.db.WithContext(ctx).Exec(alterQuery)
		if dbTx.Error != nil {
			return fmt.Errorf("could not update last block %d on chain %d: %w", blockNumber, chainID, dbTx.Error)
		}
	}
