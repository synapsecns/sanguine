...
library SwapQueryLib {
  ...
  function fillAdapterAndDeadline(
    ...
    query.routerAdapter = query.rawParams.length == 0
      ? address(0)
      : routerAdapter;
...
interface ISynapseCCTPFees {
  ...
  function tokenToSymbol(
    address token
  ) external view returns (string memory symbol);
}
...
abstract contract SynapseCCTPFees is
  SynapseCCTPFeesEvents,
  Ownable,
  ISynapseCCTPFees
{
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  function _assertCanAddSymbol(string memory symbol) internal view {
    ...
    if (symbolBytes.length <= SYMBOL_PREFIX_LENGTH)
      revert CCTPSymbolIncorrect();
...
library RequestLib {
  ...
  uint256 internal constant REQUEST_SWAP_LENGTH =
    4 * 32 + REQUEST_BASE_LENGTH + SWAP_PARAMS_LENGTH;
  ...
  function formatRequest(
    ...
    if (baseRequest.length != REQUEST_BASE_LENGTH)
    ...
    if (requestVersion == REQUEST_BASE) {
      ...
      if (swapParams.length != SWAP_PARAMS_LENGTH)
        revert IncorrectRequestLength();
  ...
  function decodeRequest(
    uint32 requestVersion,
    bytes memory formattedRequest
  ) internal pure returns (bytes memory baseRequest, bytes memory swapParams) {
    if (requestVersion == REQUEST_BASE) {
      if (formattedRequest.length != REQUEST_BASE_LENGTH)
        revert IncorrectRequestLength();
      return (formattedRequest, '');
    } else if (requestVersion == REQUEST_SWAP) {
      if (formattedRequest.length != REQUEST_SWAP_LENGTH)
        revert IncorrectRequestLength();
...
library MinimalForwarderLib {
  ...
  bytes internal constant FORWARDER_INIT_CODE =
    abi.encodePacked(hex'7f', FORWARDER_BYTECODE, hex'3d_52_60_20_3d_f3');

  /// @notice Hash of the minimal forwarder init code. Used to predict the address of a deployed forwarder.
  bytes32 internal constant FORWARDER_INIT_CODE_HASH =
    keccak256(FORWARDER_INIT_CODE);
  ...
  function forwardCallWithValue(
    ...
    returnData = forwarder.functionCallWithValue(
      abi.encodePacked(target.addressToBytes32(), payload),
      value
    );
  }
  ...
  function predictAddress(
    ...
    return
      keccak256(
        abi.encodePacked(hex'ff', deployer, salt, FORWARDER_INIT_CODE_HASH)
      ).bytes32ToAddress();
  }
}
  ...
  ISynapseCCTP
{
  ...
  constructor(ITokenMessenger tokenMessenger_, address owner_) {
    ...
    messageTransmitter = IMessageTransmitter(
      tokenMessenger_.localMessageTransmitter()
    );
  ...
  function setRemoteDomainConfig(
    ...
    if (remoteChainId == 0 || remoteChainId == block.chainid)
      revert CCTPIncorrectChainId();
    ...
    if ((remoteDomain == 0) != (remoteChainId == 1))
      revert CCTPIncorrectDomain();
    // Remote SynapseCCTP should be non-zero.
    if (remoteSynapseCCTP == address(0)) revert CCTPZeroAddress();
    remoteDomainConfig[remoteChainId] = DomainConfig(
      remoteDomain,
      remoteSynapseCCTP
    );
  }
  ...
  function receiveCircleToken(
    ...
    (bytes memory baseRequest, bytes memory swapParams) = RequestLib
      .decodeRequest(requestVersion, formattedRequest);
    (
      uint32 originDomain,
      ,
      address originBurnToken,
      uint256 amount,
      address recipient
    ) = RequestLib.decodeBaseRequest(baseRequest);
    ...
    (address tokenOut, uint256 amountOut) = _fulfillRequest(
      recipient,
      token,
      amount,
      swapParams
  ...
  function isRequestFulfilled(bytes32 requestID) external view returns (bool) {
    ...
    return
      MinimalForwarderLib.predictAddress(address(this), requestID).code.length >
      0;
  }
  ...
  function _mintCircleToken(
    ...
    bytes memory returnData = forwarder.forwardCall(
  ...
  function _fulfillRequest(
    ...
    (
      ...
      uint256 deadline,
      uint256 minAmountOut
    ) = RequestLib.decodeSwapParams(swapParams);
