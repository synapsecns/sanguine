...
func (b *BackfillSuite) TestBackfill() {
	...
	for i := range chainConfigs[0].Contracts {
		...
		err = b.eventDB.StoreLastIndexed(b.GetTestContext(), common.HexToAddress(chainConfigs[0].Contracts[i].Address), uint32(testChainID.Uint64()), lastBlock, false)
		Nil(b.T(), err)
	}
	for i := range chainConfigsV1[0].Contracts {
		//  the last block store per contract
		err = b.eventDB.StoreLastIndexed(b.GetTestContext(), common.HexToAddress(chainConfigsV1[0].Contracts[i].Address), uint32(testChainID.Uint64()), lastBlock, false)
		Nil(b.T(), err)
	}
