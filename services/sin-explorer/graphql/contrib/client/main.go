...
func main() {
	...
	err = os.Chdir(filepath.Join(root.Path, "services/sin-explorer/graphql/"))
	if err != nil {
		log.Fatalf("Error: %s", err.Error())
	}

	cfg, err := config.LoadConfig(filepath.Join(root.Path, "services/sin-explorer/graphql/gqlgen.yaml"))
	if err != nil {
		fmt.Fprintln(os.Stderr, "failed to load config", err.Error())
		os.Exit(2)
	}
	gqlgencConfig, err := clientConfig.LoadConfig(filepath.Join(root.Path, "services/sin-explorer/graphql/.gqlgenc.yaml"))
	if err != nil {
