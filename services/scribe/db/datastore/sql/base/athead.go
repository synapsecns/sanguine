...
func (s Store) RetrieveLogsFromHeadRangeQuery(ctx context.Context, logFilter db.LogFilter, startBlock uint64, endBlock uint64, page int) (logs []*types.Log, err error) {
	...
	dbTx := s.DB().WithContext(ctx).Raw(fmt.Sprintf("SELECT * FROM (%s UNION %s) ORDER BY %s DESC, %s DESC LIMIT ? OFFSET ?", subquery1, subquery2, BlockNumberFieldName, BlockIndexFieldName), PageSize, (page-1)*PageSize).Find(&dbLogs)
