...
func createScribeParameters(c *cli.Context) (eventDB db.EventDB, clients map[uint32][]service.ScribeBackend, scribeConfig config.Config, err error) {
	...
	clients = make(map[uint32][]service.ScribeBackend)
	for _, client := range scribeConfig.Chains {
		for confNum := 1; confNum <= MaxConfirmations; confNum++ {
			backendClient, err := service.DialBackend(c.Context, fmt.Sprintf("%s/%d/rpc/%d", scribeConfig.RPCURL, confNum, client.ChainID), metrics.Get())
			if err != nil {
