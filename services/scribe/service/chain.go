...
func NewChainIndexer(eventDB db.EventDB, client []ScribeBackend, chainConfig config.ChainConfig, refreshRate int, handler metrics.Handler) (*ChainIndexer, error) {
	var contractIndexers []*Indexer
	...
	var blockHeightMeterMap map[common.Address]metric.Int64Histogram
	for _, contract := range chainConfig.Contracts {
		...
		contractIndexer, err := NewIndexer(chainConfig, []config.ContractConfig{contract}, eventDB, client, handler, blockHeightMeter)
		if err != nil {
...
func (c ChainIndexer) Index(ctx context.Context, onlyOneBlock *uint64, livefill bool) error {
	...
	for i := range c.chainConfig.Contracts {
		...
		contractIndexer, err := NewIndexer(c.chainConfig, []config.ContractConfig{contract}, c.eventDB, c.client, c.handler, c.blockHeightMeters[contractAddress])
		if err != nil {
			return fmt.Errorf("could not create contract indexer: %w", err)
		}

		indexGroup.Go(func() error {
			err := c.indexToLivefill(indexCtx, onlyOneBlock, startHeight, contractIndexer)
			if err != nil {
	...
	indexGroup.Go(func() error {
		...
		livefillIndexer, err := NewIndexer(c.chainConfig, livefillContracts, c.eventDB, c.client, c.handler, livefillBlockMeter)
		if err != nil {
			return fmt.Errorf("could not create contract indexer: %w", err)
		}
		for {
...
// indexToLivefill takes a contract indexer and indexs a contract up until it reaches the livefill threshold. This function should be generally used for calling a indexer with a single contract.
func (c *ChainIndexer) indexToLivefill(parentContext context.Context, onlyOneBlock *uint64, startHeight uint64, indexer *Indexer) error {
	timeout := time.Duration(0)
	b := &backoff.Backoff{
		Factor: 2,
		Jitter: true,
