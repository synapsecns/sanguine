...
func NewIndexer(chainConfig config.ChainConfig, contractConfig []config.ContractConfig, eventDB db.EventDB, client []ScribeBackend, handler metrics.Handler, blockMeter otelMetrics.Int64Histogram) (*Indexer, error) {
	...
	if len(contractConfig) == 1 && contractConfig[0].RefreshRate > 0 {
		refreshRate = contractConfig[0].RefreshRate
	}

	var contracts []common.Address
	for _, contract := range chainConfig.Contracts {
		contracts = append(contracts, common.HexToAddress(contract.Address))
	}
