...
func (s *ScribeSuite) TestSimulatedScribe() {
	...
	for _, chainConfig := range scribeConfig.Chains {
		for _, contractConfig := range chainConfig.Contracts {
			...
			lastIndexed, err := s.testDB.RetrieveLastIndexed(s.GetTestContext(), common.HexToAddress(contractConfig.Address), chainConfig.ChainID)
			Nil(s.T(), err)
...
func (s *ScribeSuite) TestLivefillParity() {
	...
	scribe, err := scribe.NewScribe(s.testDB, clients, scribeConfig, s.nullMetrics)
	Nil(s.T(), err)
