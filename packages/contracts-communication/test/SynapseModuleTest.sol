...
contract SynapseModuleTest is Test, ISynapseModuleEvents {
  ...
  function getMockEntry(
    address writer,
    uint256 nonce
  ) internal pure returns (InterchainEntry memory entry) {
    return
      InterchainEntry({
  ...
  function test_requestVerification_insufficientFee() public {
    ...
    synapseModule.requestVerification{value: expectedFee - 1}(
      DST_CHAIN_ID,
    ...
    for (uint i = 0; i < verifiers.length; i++) {
      if (plannedValidSignatures < expectedThreshold - 1) {
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(
          verifiersPrivateKeys[i],
          keccak256(abi.encode(entry))
        );
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(
          verifiersPrivateKeys[i],
          keccak256(abi.encode('invalid message'))
        );
        signatures[i] = abi.encodePacked(r, s, v);
      }
    }
