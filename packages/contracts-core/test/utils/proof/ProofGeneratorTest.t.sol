...
import {BaseTree, MerkleTree} from "../../../contracts/libs/MerkleTree.sol";
...
contract ProofGeneratorTest is Test {
    ...
    function _checkGenerateProofs() internal {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            assertEq(MerkleTree.proofRoot(i, leafs[i], proof, ORIGIN_TREE_HEIGHT), root, "!proof");
        }
        // Cool side effect: could prove message non-inclusion at next index
        {
            ...
            assertEq(MerkleTree.proofRoot(index, bytes32(0), proof, ORIGIN_TREE_HEIGHT), root, "!proof");
        }
