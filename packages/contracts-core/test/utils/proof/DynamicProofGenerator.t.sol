...
import {MerkleTree, AGENT_TREE_HEIGHT} from "../../../contracts/libs/MerkleTree.sol";
...
contract DynamicProofGenerator is ProofCutter {
    ...
    function update(uint256 index, bytes32 newValue) external {
        ...
        for (uint256 h = 1; h <= AGENT_TREE_HEIGHT; ++h) {
            // Traverse to parent
            index >>= 1;
            merkleTree[h][index] = MerkleTree.getParent(merkleTree[h - 1][2 * index], merkleTree[h - 1][2 * index + 1]);
        }
    }
