...
contract DeployMessaging003SynChainScript is DeployMessaging003BaseScript {
        ...
        returns (address deployment, bytes memory constructorArgs)
    {
        ...
        deployment = factoryDeploy(
            agentManagerName(),
            type(BondingManager).creationCode,
            constructorArgs
        );
    ...
    function _addAgents() internal override {
        ...
        for (uint256 i = 0; i < domains.length; ++i) {
            ...
            address[] memory agents = globalConfig.readAddressArray(
            ...
            for (uint256 j = 0; j < agents.length; ++j) {
                ...
                if (BondingManager(agentManager).agentStatus(agent).flag != AgentFlag.Unknown) {
                    console.log(
                        "   [address: %s] [domain: %s] skipped (already added)",
                        agent,
                        domain
                    );
                    continue;
        ...
        for (uint256 i = 0; i < domains.length; ++i) {
            ...
            string[] memory agentsStr = globalConfig.readStringArray(
                string.concat(".agents.", domain.toString())
            );
            for (uint256 j = 0; j < agents.length; ++j) {
                address agent = agents[j];
