...
abstract contract BaseClient is IMessageRecipient {
     ...
     */
    function _sendBaseMessage(uint32 destination_, bytes memory tipsPayload, Request request, bytes memory content)
        internal
    {
        ...
        InterfaceOrigin(origin).sendBaseMessage{value: msg.value}(
            destination_, recipient, optimisticPeriod(), tipsPayload, Request.unwrap(request), content
        );
    }
}
