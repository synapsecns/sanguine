...
export function useSwapPoolDeposit(poolName, address, chainId) {
  ...
  for (const [symbol, inputStr] of _.toPairs(inputState)) {
    sanitizedInputState[symbol] = cleanNumberInput(inputStr)
  }

  let numericalInputState = {}
  for (const [symbol, inputStr] of _.toPairs(inputState)) {
    numericalInputState[symbol] = Number(cleanNumberInput(inputStr))
  }
  ...
  async function calculateMaxDeposits() {
    if (swapContract == null || poolData == null || address == null) {
      return
    }
  ...
  useEffect(() => {
    ...
    // }
  }, [inputState, swapContract, address])
