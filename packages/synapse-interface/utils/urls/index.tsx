...
export const   TERMS_OF_SERVICE_PATH = 'https://explorer.synapseprotocol.com/terms'
export const   PRIVACY_POLICY_PATH = 'https://explorer.synapseprotocol.com/privacy'

export const SYNAPSE_PFP_PATH = '/returntomonke'


export function getPoolUrl({ token, poolRouterIndex }: { token?: any, poolRouterIndex?: number }) {
...
export function getExplorerTxUrl({ hash, data, chainId=56, type="tx" }: { hash?: string, data?: string, chainId?: number, type?: string }) {
...
export const CAREERS_URL  = 'https://synapseprotocol.notion.site/synapseprotocol/Synapse-Job-Board-3851178379bf45b2b47c6ec8bf9d6753'
export const MIRROR_URL   = 'https://synapse.mirror.xyz/'

export const HOW_TO_BRIDGE_URL = 'https://docs.synapseprotocol.com/how-to/bridge'
export const HOW_TO_SWAP_URL   = 'https://docs.synapseprotocol.com/how-to/swap'
export const HOW_TO_STAKE_URL  = 'https://docs.synapseprotocol.com/how-to/provide-liquidity'
...
function getSushiSwapUrl({fromCoin, toCoin, chainId }: { fromCoin?: any, toCoin?: any, chainId: number }) {
...
function getTraderJoeSwapUrl({ fromCoin, toCoin, chainId }: { fromCoin?: any, toCoin?: any, chainId: number }) {
  const inputCurrency = fromCoin?.addresses?.[chainId] ?? ""
  const outputCurrency = toCoin?.addresses?.[chainId] ?? ""
...
function getFirebirdSwapUrl({ fromCoin, toCoin, chainId }: { fromCoin?: any, toCoin?: any, chainId: number }) {
  const inputCurrency = fromCoin?.addresses?.[chainId] ?? ""
  const outputCurrency = toCoin?.addresses?.[chainId] ?? ""
...
function getUniswapSwapUrl({ fromCoin, toCoin, chainId }: { fromCoin?: any, toCoin?: any, chainId: number }) {
  const inputCurrency = fromCoin?.addresses?.[chainId] ?? ""
  const outputCurrency = toCoin?.addresses?.[chainId] ?? ""
...
export function getBuySynUrl({ chainId }: { chainId: number }) {
  ...
  switch (chainId) {
    case ChainId.ETH: return getUniswapSwapUrl(params)
    case ChainId.AVALANCHE: return getTraderJoeSwapUrl(params)
    case ChainId.FANTOM: return getFirebirdSwapUrl(params)
    default:
...
export const BRIDGESYN_ANALYTICS_API = "https://analytics-api.bridgesyn.com/api/v1/analytics"
