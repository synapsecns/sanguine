...
import { DOG, LINK, GOHM, HIGHSTREET, JUMP, NFD, NEWO, VSTA, GMX, SDT, UNIDX, USDB, SFI, H2O, L2DAO, PLS, AGEUR } from '@constants/tokens/mintable'
import { SYN, NUSD, NETH, TOKEN_HASH_MAP, WETH, FRAX, SYN_FRAX, UST,  KLAYTN_USDT, KLAYTN_USDC, KLAYTN_DAI, WBTC, KLAYTN_WETH, DOGECHAIN_BUSD, NOTE } from '@constants/tokens/basic'
...
//  const dogechainSynapseContract     = useGenericSynapseContract(ChainId.DOGECHAIN)
  ...
  const getAllTransactions = useCallback( _.throttle( () => {
    ...
    getTerraBridgeEvents({ account, terraAddress })
      .then(updateTransactions)
...
async function getBridgeZapsOnChain({ library, synapseContract, account, terraAddress, chainId, transactions, addressesForAccount }) {
  ...
  const evmAddrsForAccount = (addressesForAccount ?? []).filter(s => s.slice(0, 2) == "0x")
  const terraAddrsForAccount = (addressesForAccount ?? []).filter(s => s.slice(0, 6) == "terra")
  ...
  const pastEventsByHandle = await Promise.all(
    synapseContractEventHandles.map(eventHandle => {
      ...
      if ((eventHandle == "TokenRedeemV2") && terraAddress) {
        const terraAddrs = [...terraAddrsForAccount, terraAddress].map(addr => bech32.decode(addr).words)
...
function mergeTxObj(chainId, eventObj, timestampObj, txReceipt) {
  ...
  if (
    [SYN, LINK, HIGHSTREET, DOG, JUMP, FRAX, NFD, GOHM, AGEUR, H2O, L2DAO, PLS, NEWO, VSTA, SFI, SDT, UNIDX, USDB, GMX, UST, KLAYTN_USDT, KLAYTN_USDC, KLAYTN_DAI, WBTC, KLAYTN_WETH, DOGECHAIN_BUSD, NOTE].map(t => _.toLower(t.addresses[chainId])).includes(swapTokenAddr)
  ) {
  ...
  if (
    (_.toLower(txReceipt.logs[0]?.address) == _.toLower(GMX.addresses[ChainId.ARBITRUM]) )||
    (_.toLower(txReceipt.logs[1]?.address) == _.toLower(GMX.addresses[ChainId.AVALANCHE]))
  ) {
    inputTokenAmount = txReceipt.logs[1].data
  } else if (_.toLower(txReceipt.logs[1].address) == _.toLower(UST.addresses[chainId])) {
  ...
  let inputTokenAddr
  if (_.toLower(txReceipt.logs[1].address) == _.toLower(GMX.addresses[ChainId.AVALANCHE])) {
