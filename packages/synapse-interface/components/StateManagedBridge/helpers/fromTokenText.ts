...
export const fromTokenText = ({
  ...
  if (fromChainId && fromToken === null && toChainId && toToken === null) {
    return `All ${fromChainName} tokens you can send to ${toChainName}`
  }
  ...
  if (fromChainId && fromToken === null && toChainId === null && toToken) {
    return `All ${fromChainName} tokens you can send and receive ${toToken.symbol}`
  }

  if (fromChainId === null && fromToken && toChainId === null && toToken) {
    return `All tokens you send and receive ${toToken.symbol}`
  }

  if (fromChainId === null && fromToken === null && toChainId && toToken) {
    return `All tokens you can send and receive ${toToken.symbol} on ${toChainName}`
  }

  if (fromChainId && fromToken === null && toChainId && toToken) {
    return `All ${fromChainName} tokens you can send and receive ${toToken.symbol} on ${toChainName}`
  }
