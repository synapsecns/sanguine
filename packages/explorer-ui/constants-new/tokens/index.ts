...
import * as all from '@constants-new/tokens/bridgeable'
import * as allAux from '@constants-new/tokens/auxilliary'
import * as allPool from '@constants-new/tokens/poolMaster'
import { GMX, ETH, USDC, USDT, WETH } from '@constants-new/tokens/bridgeable'
import { SYN_ETH_SUSHI_TOKEN } from '@constants-new/tokens/sushiMaster'
...
const getTokensByType = (tokens: Record<string, Token>): TokensByChain => {
  const tokensByType: TokensByChain = {}
  Object.entries(tokens).map(([key, token]) => {
    for (const cID of Object.keys(token.addresses)) {
      ...
      if (!tokensByType[cID]) {
        tokensByType[cID] = [token]
      } else {
        if (!tokensByType[cID]?.includes(token)) {
          tokensByType[cID] = [...tokensByType[cID], token]
        }
      }
    }
  })
...
export const BRIDGABLE_TOKENS = getTokensByType(all)
export const AUXILLARY_TOKENS = getTokensByType(allAux)

const getTokenHashMap = (tokensByType: TokensByChain) => {
  const tokenHashMap = {}

  for (const [chainId, tokensOnChain] of Object.entries(tokensByType)) {
...
const bridgeableTokensHashMap = getTokenHashMap(BRIDGABLE_TOKENS)
const auxillaryTokensHashMap = getTokenHashMap(AUXILLARY_TOKENS)

const combinedHashMap = {}

for (const chainId of Object.keys({
  ...bridgeableTokensHashMap,
  ...auxillaryTokensHashMap,
})) {
  combinedHashMap[chainId] = {
    ...bridgeableTokensHashMap[chainId],
    ...auxillaryTokensHashMap[chainId],
  }
}

combinedHashMap[CHAINS.AVALANCHE.id][
  GMX.wrapperAddresses[CHAINS.AVALANCHE.id]
] = GMX
Object.keys(WETH.addresses).map((chain) => {
  combinedHashMap[chain][WETH.addresses[chain]] = ETH
})

export const TOKEN_HASH_MAP = combinedHashMap
