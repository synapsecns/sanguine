...
export function ChartFull(
  ...
  const currentTooltipIndexVar = 0
  ...
  const returnChainData = () => {
    const items = Object.keys(dailyDataArr?.[currentTooltipIndex]).map(
      (key) => {
        return [key, dailyDataArr?.[currentTooltipIndex][key]]
      }
    )
    items.sort((first, second) => {
      return second[1] - first[1]
    })
    const keys = items.map((e) => {
      return e[0]
    })
    return keys
  }
  return (
    <div className="grid grid-cols-4 gap-4">
      <div className="col-span-1 w-[100%]">
        <p className="text-lg font-bold text-white pl-2 pt-4 ">
          Date: {formatDate(dailyDataArr?.[currentTooltipIndex]?.date)}
        </p>
        <table className="min-w-full">
          ...
          {loadingRankedChains ? (
            <tbody>
              {' '}
              {Object.values(CHAIN_ID_NAMES_REVERSE).map((i) => (
                <tr key={i}>
                  <td className="w-[70%]">
                    {' '}
                    <div className="h-3 w-full mt-4 bg-slate-700 rounded animate-pulse"></div>
                  </td>
                  <td className="w-[30%]">
                    <div className="h-3 w-full mt-4 bg-slate-700 rounded animate-pulse"></div>
                  </td>
            ...
            <tbody>
              {currentTooltipIndex >= 0 && dailyDataArr?.[currentTooltipIndex]
                ? returnChainData().map((key, i) => {
                    return dailyDataArr[currentTooltipIndex][key] > 0 ? (
                      <tr key={i} className=" rounded-md w-[100%]">
                        <td className="w-[70%]">
                          {key === 'total' ? (
                        ...
                        <td className="w-fit ">
                          <div className="ml-1 mr-2 self-center">
                            <p className="whitespace-nowrap px-2  text-sm  text-white">
                              {formatUSD(
                                dailyDataArr[currentTooltipIndex][key]
                              )}
                            </p>
                          </div>
                        </td>
      ...
      <div className="col-span-3 ">
        ...
        {loadingDailyData ? (
          <div className="flex justify-center align-center w-full my-[240px]">
            <div className="animate-spin w-[39px]">
              <SynapseLogoSvg />
            </div>
          </div>
        ) : (
