...
export function Stats({ bridgeVolume, transactions, addresses, setChartType, allTime }) {
...
export function AllTimeStatCard({ onClick, title, children, active = false, duration = "All-Time" }) {
  const activeClass = active ? 'opacity-100' : 'opacity-20 hover:opacity-100'
  return (
    <Card
      className={`px-0 pb-2 space-y-3 text-white bg-transparent cursor-pointer ${activeClass}`}
