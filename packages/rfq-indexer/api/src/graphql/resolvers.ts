...
const qDeposits = () => {
  return db.selectFrom('BridgeRequestEvents')
    .select([
...
const qRelays = () => {
  return db.selectFrom('BridgeRelayedEvents')
    .select([
...
const qProofs = () => {
  return db.selectFrom('BridgeProofProvidedEvents')
    .select([
...
const qClaims = () => {
  return db.selectFrom('BridgeDepositClaimedEvents')
    .select([
...
const qRefunds = () => {
  return db.selectFrom('BridgeDepositRefundedEvents')
    .select([
...
function nest_results(sqlResults: any[]) {
  return sqlResults.map((transaction: any) => {
    ...
    for (const [key, value] of Object.entries(transaction)) {
      if (key.startsWith('transactionId')) {
        ...
        bridgeRequest[key.replace('_refund', '')] = value;
...
const resolvers = {
  Query: {
    events: async (_: any, { first = 10, after, filter }: { first?: any; after?: any; filter?: any }) => {
      let query = db.selectFrom('BridgeRequestEvents')
        .select(['BridgeRequestEvents.id', 'BridgeRequestEvents.transactionId', 'BridgeRequestEvents.blockNumber', 'BridgeRequestEvents.blockTimestamp', 'BridgeRequestEvents.transactionHash', 'BridgeRequestEvents.originChainId', 'BridgeRequestEvents.originChain'])
        .unionAll(
          db.selectFrom('BridgeRelayedEvents')
            .select(['BridgeRelayedEvents.id', 'BridgeRelayedEvents.transactionId', 'BridgeRelayedEvents.blockNumber', 'BridgeRelayedEvents.blockTimestamp', 'BridgeRelayedEvents.transactionHash', 'BridgeRelayedEvents.originChainId', 'BridgeRelayedEvents.originChain'])
        )
        .unionAll(
          db.selectFrom('BridgeProofProvidedEvents')
            .select(['BridgeProofProvidedEvents.id', 'BridgeProofProvidedEvents.transactionId', 'BridgeProofProvidedEvents.blockNumber', 'BridgeProofProvidedEvents.blockTimestamp', 'BridgeProofProvidedEvents.transactionHash', 'BridgeProofProvidedEvents.originChainId', 'BridgeProofProvidedEvents.originChain'])
        )
        .unionAll(
          db.selectFrom('BridgeDepositRefundedEvents')
            .select(['BridgeDepositRefundedEvents.id', 'BridgeDepositRefundedEvents.transactionId', 'BridgeDepositRefundedEvents.blockNumber', 'BridgeDepositRefundedEvents.blockTimestamp', 'BridgeDepositRefundedEvents.transactionHash', 'BridgeDepositRefundedEvents.originChainId', 'BridgeDepositRefundedEvents.originChain'])
        )
        .unionAll(
          db.selectFrom('BridgeDepositClaimedEvents')
            .select(['BridgeDepositClaimedEvents.id', 'BridgeDepositClaimedEvents.transactionId', 'BridgeDepositClaimedEvents.blockNumber', 'BridgeDepositClaimedEvents.blockTimestamp', 'BridgeDepositClaimedEvents.transactionHash', 'BridgeDepositClaimedEvents.originChainId', 'BridgeDepositClaimedEvents.originChain'])
        )
  ...
  BridgeEvent: {
    __resolveType(obj: any) {
      ...
      // For example:
      if ('sender' in obj) return 'BridgeRequestEvent'
      if ('relayer' in obj && 'to' in obj) return 'BridgeRelayedEvent'
      if ('relayer' in obj && !('to' in obj)) return 'BridgeProofProvidedEvent'
      if ('to' in obj && 'token' in obj) return 'BridgeDepositRefundedEvent'
      if ('relayer' in obj && 'to' in obj && 'token' in obj) return 'BridgeDepositClaimedEvent'
      return null
    },
  },
}
