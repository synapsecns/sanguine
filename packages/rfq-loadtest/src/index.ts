import { Chain, createPublicClient, createWalletClient, formatUnits, http, parseUnits, publicActions, PublicClient, WalletClient } from 'viem';
...
try {
    ...
    if (typeof config.VOLLEY_MILLISECONDS_BETWEEN !== 'number' || typeof config.VOLLEY_MIN_COUNT !== 'number' || typeof config.VOLLEY_MAX_COUNT !== 'number') {
    ...
    Object.entries(config.TEST_ROUTES).forEach(([route, details]: [string, any]) => {
        if (typeof details !== 'object' || details === null) {
            throw new Error(`Invalid configuration for route: ${route}`);
        }
        if (typeof details.fromChainId !== 'number' || typeof details.toChainId !== 'number' || typeof details.testDistributionPercentage !== 'number') {
            throw new Error(`Invalid configuration values for route: ${route}`);
        }
    });

} catch (error: any) {
    throw new Error(`Failed to parse ${configFilePath}. Check your syntax, structure, data, and for duplicates. \n${error.message}`);
}
...
const vChains: any = {};
Object.entries(config.CHAINS).forEach(([chainId, chainConfig]: [string, any]) => {
    const viemChain = Object.values(viemChains).find(chain => chain.id === parseInt(chainId));
    if (!viemChain) {
        throw new Error(`No viem chain config found for chain ID ${chainId}. Bump viem version or add manually`);
    }
...
const walletAccount = privateKeyToAccount(privateKey, {nonceManager: nonceManager});
...
Object.keys(vChains).forEach((chainId:string) => {
    ...
    ]).then(([fastBridgeAddr, blockNumber_Read, blockNumber_Sim, blockNumber_Write]: [string, bigint, bigint, bigint]) => {
        print(`Connected to chain ID ${chainId.toString().padStart(7)}. FastBridge at: ${fastBridgeAddr.slice(0,6)}... Current block height: ${blockNumber_Read}`);
    });
});

var cachedResponseRFQ: any
...
async function checkBals() {
    for (;;) {
        await Promise.all(Object.keys(vChains).map(async (chainId: string) => {
            const chain = vChains[chainId];
            try {
                const balance = await chain.vCliRead.getBalance({address: walletAccount.address});
                chain.balanceRaw = balance
                chain.balanceUnits = formatUnits(balance, 18); 
            } catch (error: any) {
                print(`Error fetching balance for chain ID ${chainId}: ${error.message}`);
            }
        }));
...
async function bridgeLooper() {
    for (;;) {
        ...
        const rebalToChain:any = Object.values(vChains).find((chain: any) => chain.balanceUnits < minGasUnits);

        if(rebalToChain)
        {

            const rebalFromChain:any = Object.values(vChains).reduce((prev:any, current:any) => {
            ...
            {
                print(`${rebalLabel} Last action was identical (${lastAction}). Not repeating. Re-evaluating momentarily...`)
            ...
            if(rebalFromChain.balanceUnits<(rebalToUnits * 1.1))
            {
                ...
                print(`${rebalLabel} - Insuff Funds on From Chain ${rebalFromChain.balanceUnits}. Ending tests.`)
                await delay(60_000)
                return
            }

            await sendBridge(rebalFromChain, rebalToChain, Number(rebalAmount.toFixed(18)), false, rebalLabel);
        ...
        const totalPercentage:number = Object.values(config.TEST_ROUTES).reduce((acc:number, route:any) => acc + route.testDistributionPercentage, 0);
        const randomizer:number = getRandomInt(1, totalPercentage);
        let cumulative = 0;

        for (const [route, details] of Object.entries(config.TEST_ROUTES) as [string, any][]) {
        ...
        const countToSend = getRandomInt(config.VOLLEY_MIN_COUNT, config.VOLLEY_MAX_COUNT);
        const printLabel = `%ts Batch${(sendCounter + countToSend).toString().padStart(5, '0')} of ${countToSend} : ${fromChain.id.toString().padStart(7)} >> ${toChain.id.toString().padEnd(7)}`
        ...
        for (let i = 0; i < countToSend; i++) {
            ...
            sendBridge(fromChain, toChain, Number((testBridgeAmountUnits + (sendCounter / 100000000000)).toFixed(18)), true, printLabel);
            sendCounter++;
            await delay(50);
        }
...
async function getRequestParams(fromChain: any, toChain: any, sendAmountUnits: number)
{

    // 480 is not supported currently on the API - using Opti/Base as proxies. Can be improved later as needed.
    if(toChain.id==480) toChain = fromChain.id==8453 ? vChains['10'] : vChains['8453']
    if(fromChain.id==480) fromChain = toChain.id==8453 ? vChains['10'] : vChains['8453']

    const requestURL = `https://api.synapseprotocol.com/bridge?fromChain=${fromChain.id}&toChain=${toChain.id}&fromToken=${'0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'}&toToken=${'0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'}&amount=${sendAmountUnits}&destAddress=${walletAccount.address}`
    ...
    response = await axios.request(
        ...
        if((response.data?.length ?? 0) == 0) throw new Error(`No data returned from api`)

        responseRFQ = Array.isArray(response.data) ? response.data.find((item:any) => item.bridgeModuleName === "SynapseRFQ") : null;
        if (!responseRFQ) {
    ...
    {
        throw new Error(`RFQ Api Fail: ${error.message.substr(0,50)} -- ${requestURL}`)
    }
...
async function looper_getRequestParams()
{
    for(;;)
    {
        ...
        cachedResponseRFQ = await getRequestParams(vChains['8453'], vChains['10'], testBridgeAmountUnits)
...
async function sendBridge(fromChain: any, toChain: any, sendAmountUnits: number, useCachedRequest: boolean, printLabel: string)
{
    const sendAmountRaw = parseUnits(sendAmountUnits.toString(), 18)

    printLabel = printLabel + ` ${sendAmountUnits} ETH`

    const _responseRFQ = useCachedRequest ? cachedResponseRFQ : await getRequestParams(fromChain, toChain, sendAmountUnits)
