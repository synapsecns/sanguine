...
app.get('/bridgeQuote', async (req, res) => {
  // Access query params
  const query = req.query
...
app.get('/swap', async (req, res) => {
  ...
  //To address
  const toAddress = String(query.toAddress)
  ...
  if (
    ...
    res.send(
      ...
      <h1>Invalid Params</h1>
      <hr/>
      <b>Ensure that your request matches the following format: /swap?chain=1&fromToken=USDC&toToken=DAI&amount=100&toAddress=0x7E7A0e201FD38d3ADAA9523Da6C109a07118C96a</b>
      <h2>Available Tokens (symbols to use)</h2>
      ${tokenHtml}`
    )
    return
