...
check_field() {
  ...
  if [ "$actual_value" != "$expected_value" ]; then
    echo "    ❌ Gas.zip calldata.$field_name INCORRECT for $chainName" >&2
...
echo "Testing 0.001 ETH from Ethereum Mainnet (chain ID: $ETH_CHAIN_ID)"
for chainName in "${CHAIN_NAMES[@]}"; do
  ...
  # Check that Gas.zip quote exists
  quote=$(echo "$quotes" | jq '.[] | select(.bridgeModuleName == "Gas.zip")')
  if [ -z "$quote" ]; then
    echo "    ❌ Gas.zip quote not found for $chainName" >&2
    exit 1
  fi
  ...
  echo "    ✅ Gas.zip quote found for $chainName: $maxAmountOut"
done
