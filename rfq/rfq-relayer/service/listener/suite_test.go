...
// NewListenerSuiteSuite creates a new ListenerSuite.
func NewListenerSuiteSuite(tb testing.TB) *ListenerSuite {
	tb.Helper()
	return &ListenerSuite{
...
func (t *ListenerSuite) SetupTest() {
...
func (t *ListenerSuite) SetupSuite() {
...
func (t *ListenerSuite) setupMysqlDB() {
	...
	Nil(t.T(), err)
	// Add the db to the Listener suite
	t.dbs = append(t.dbs, mysqlStore)
}

// RunOnAllDBs runs the test function on all dbs available.
func (t *ListenerSuite) RunOnAllDBs(testFunc func(testDB db.TestDB)) {
	t.T().Helper()
	wg := sync.WaitGroup{}
...
// TestDBSuite tests the Listener suite.
func TestListenerSuiteSuite(t *testing.T) {
	suite.Run(t, NewListenerSuiteSuite(t))
}
