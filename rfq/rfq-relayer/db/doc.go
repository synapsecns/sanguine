...
├── read_test.go: All the tests for the read operations for the database (its testing all the functions in the `DBReader` interface in db.go).
├── write_test.go: All the tests for the write operations for the database (its testing all the functions in the `DBWriter` interface in db.go).
...
│ ├── base: Holds all the implementations of `DBReader` and `DBWriter`. Also has the database init functionality.
