...
func main() {
	...
	repoOwner, repoPath := githubparser.ParseGithubRepository(os.Getenv("GITHUB_REPOSITORY"))
	token := githubactions.GetInput("github_token")

	// we also parse the source and target repositories
	destOwner, destRepo := githubparser.ParseGithubRepository(githubactions.GetInput("destination_repo"))
	// and the tag to copy
